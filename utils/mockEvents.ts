import { CalendarEvent, EventParticipant, Market } from "@/types/event";
import {
  TBD,
  TEAM_SEPARATOR,
  LEAGUE_NAME,
...
// ── Mock markets ────────────────────────────────────────────
// Reusable market templates for testing scenarios.
...
// ── Known teams catalog ─────────────────────────────────────
// Provides name + countryCode pairs for quick reference.
// CountryCode is resolved via countryMapping; this catalog is
// just a convenient shorthand for mock configuration.
...
/**
 * Factory for creating mock CalendarEvents.
 * All fields have sensible defaults (TBD teams, no markets, not live).
 * Pass overrides to configure specific scenarios.
 */
...
/**
 * Creates N events at the same date/time to test simultaneous rendering.
 * Each event gets a unique ID suffix.
 */
...
// ── Knockout fixtures ───────────────────────────────────────
// Declarative data — each fixture optionally includes team info
// for "decided" matchups. TBD fixtures omit home/away.
...
const KNOCKOUT_FIXTURES: KnockoutFixture[] = [
  ...
  // Dates preserved from original schedule.
  // Some slots naturally share the same kickoff (simultaneous rendering).
